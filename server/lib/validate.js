...
function validateBookACall(body) {
  ...
  return { errors, data: { name, email, website: trimStr(body.website, MAX_URL), message: trimStr(body.message, MAX_MESSAGE), phone: trimStr(body.phone, 50) } };
}
...
function validateLead(body) {
  ...
  const name = trimStr(body.name, MAX_NAME);
  return { errors, data: { email, name, source } };
}
